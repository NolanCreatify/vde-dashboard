...
// ── CONFIG ─────────────────────────────────────────────────────────────────────
// After you deploy your Google Apps Script, paste the URL here:
const APPS_SCRIPT_URL = "YOUR_APPS_SCRIPT_URL_HERE";

const VDE_NAMES = [
  "VDE 1", "VDE 2", "VDE 3", "VDE 4", "VDE 5", "VDE 6", "VDE 7"
];
...
async function submitLog(entry) {
  ...
  localStorage.setItem("vde-logs-v1", JSON.stringify(updated));
...
const Tag = ({ children, color = "green" }) => {
  ...
  return (
    <span className={`text-xs px-2 py-0.5 rounded border font-mono ${p[color]}`}>
      {children}
    </span>
  );
};
...
const Stat = ({ label, value, sub, accent = false }) => (
  <div className="flex flex-col gap-0.5">
    ...
    <span className={`text-2xl font-black font-mono ${accent ? "text-emerald-400" : "text-white"}`}>
      {value}
    </span>
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-6 py-20">
        ...
        <button
          onClick={() => {
            setSubmitted(false);
            setForm({ vde: "", date: today(), totalVideos: "", onTime: "", overtime: "", beforeNoon: "", afterNoon: "", blockers: [], otherBlocker: "", wins: "" });
          }}
  ...
  return (
    <div className="max-w-lg mx-auto flex flex-col gap-6 pb-10">
      <div>
        ...
        <select className={inp} value={form.vde} onChange={(e) => set("vde", e.target.value)}>
          <option value="">— Select —</option>
          {VDE_NAMES.map((n) => <option key={n}>{n}</option>)}
        </select>
      </div>
...
function Dashboard({ logs, onRefresh, loading }) {
  ...
  const teamStats = (set) => ({
    total: set.reduce((s, l) => s + l.totalVideos, 0),
    onTime: set.reduce((s, l) => s + l.onTime, 0),
    ot: set.reduce((s, l) => s + l.overtime, 0),
    bn: set.reduce((s, l) => s + l.beforeNoon, 0),
    an: set.reduce((s, l) => s + l.afterNoon, 0),
    avgPerVDE: set.length
      ? (set.reduce((s, l) => s + l.totalVideos, 0) / new Set(set.map((l) => l.vde)).size).toFixed(1)
      : "0.0",
  });
  ...
  const byVDE = (set) => {
    const map = {};
    for (const l of set) {
      if (!map[l.vde]) map[l.vde] = { total: 0, onTime: 0, overtime: 0, beforeNoon: 0, afterNoon: 0, days: 0, blockers: [] };
  ...
  const renderToday = () => {
    ...
    return (
      <div className="flex flex-col gap-6">
        ...
        <Card>
          <p className="text-xs text-zinc-400 uppercase tracking-widest mb-4">VDE Submissions Today</p>
          {VDE_NAMES.map((name) => {
            const d = vdeData[name];
            if (!d) return (
  ...
  const renderLeaderboard = () => {
    const vdeData = byVDE(weekLogs);
    const sorted = Object.entries(vdeData).sort((a, b) => b[1].total - a[1].total);
    const medals = ["🥇", "🥈", "🥉"];
    return (
        ...
        {sorted.map(([name, d], i) => (
          <Card key={name} className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-zinc-500">{d.days} day{d.days !== 1 ? "s" : ""} logged</p>
            </div>
  ...
  const renderWins = () => {
    const wins = [...logs].filter((l) => l.wins?.trim()).reverse().slice(0, 30);
    return (
